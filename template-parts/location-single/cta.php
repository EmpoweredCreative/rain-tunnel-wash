<?php
/**
 * Location Single - CTA (same as service single)
 *
 * @package RainTunnelWash
 */

$heading   = get_field('location_single_cta_heading');
$text      = get_field('location_single_cta_text');
$button    = get_field('location_single_cta_button');
$btn_text  = get_field('location_single_cta_button_text');
$bg_image  = get_field('location_single_cta_background_image');
$bg_url    = $bg_image && !empty($bg_image['url']) ? $bg_image['url'] : '';

if (!$heading && !$button && !$btn_text) return;

$button_label  = $btn_text ? trim($btn_text) : (is_array($button) && !empty($button['title']) ? $button['title'] : '');
