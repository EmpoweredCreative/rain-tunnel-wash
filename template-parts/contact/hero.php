<?php
/**
 * Contact Page - Hero (same structure as home page)
 *
 * @package RainTunnelWash
 */

$badge        = get_field( 'contact_hero_badge' );
$tagline_raw  = get_field( 'contact_hero_tagline' );
$tagline      = ( $tagline_raw !== null && trim( (string) $tagline_raw ) !== '' ) ? trim( (string) $tagline_raw ) : '';
$title        = get_field( 'contact_hero_title' ) ?: get_the_title();
$subtitle     = get_field( 'contact_hero_subtitle' ) ?: '';
$desc         = get_field( 'contact_hero_description' ) ?: '';
$hero_image   = get_field( 'contact_hero_background' );
$cta1_text_raw = get_field( 'contact_hero_cta1_text' );
$cta1_link_raw = get_field( 'contact_hero_cta1_link' );
$cta2_text_raw = get_field( 'contact_hero_cta2_text' );
$cta2_link_raw = get_field( 'contact_hero_cta2_link' );
...
if ( $hero_image && ! empty( $hero_image['url'] ) ) {
    $bg_image = ! empty( $hero_image['id'] ) && wp_get_attachment_image_src( (int) $hero_image['id'], 'full' )
        ? wp_get_attachment_image_url( (int) $hero_image['id'], 'full' )
        : $hero_image['url'];
}
